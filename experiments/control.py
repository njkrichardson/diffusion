...
parser.add_argument("--num-timesteps", "-t", type=int, default=500, help="Number of steps taken to encode/diffuse data into noise codes.")
...
def main(args): 
    ...
    transform, reverse_transform = close_transform(observation_dimension)
    dataset: Dataset = MonochromeDataset(num_observations, observation_dimension, device, transform)
    dataloader: DataLoader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True) 
    log.info(f"Sampled {num_observations=} of {observation_dimension=}")
    ...
    # instantiate decoder 
    decoder: Module = UNet(observation_dimension) 
    decoder.to(device) 
    ...
    for epoch in range(args.num_epochs): 
        ...
        if epoch % args.report_every == 0: 
            ...
            samples = sample(decoder, (1, 1, observation_dimension, observation_dimension)) 
