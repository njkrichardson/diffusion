...
class Trainer(object):
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            ...
            log_fn(f"Iteration [{self.step:05d}/{self.train_num_steps:05d}]: {loss.item():0.5f}")

            if writer is not None: 
                writer.scalar("Loss", loss.item(), step=self.step)
...
#                video_tensor_to_gif(one_gif, video_path)
                if writer is not None: 
                    writer.gif(f"Sample_{0}", one_gif.permute(1, 2, 3, 0), step=self.step) 
